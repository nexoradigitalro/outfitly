...
export function ItemCard({ item }: { item: ClosetItem }) {
  return (
    <div className="flex flex-col gap-2.5 rounded-2xl border border-border bg-card p-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_10px_20px_-12px_rgba(0,0,0,0.5)] transition-transform hover:-translate-y-0.5">
      <div
        className="relative flex aspect-[1/0.92] items-center justify-center rounded-[11px]"
        style={{
          background:
            "radial-gradient(120% 120% at 25% 15%, rgba(255,255,255,0.09), rgba(255,255,255,0.03) 60%)",
        }}
      >
        {/* Real photo path (image_thumb_path) lands with the M1 capture
            pipeline (docs/ARCHITECTURE.md §12) — until then every item
            renders this tinted placeholder, which is also the correct
            fallback for an item still mid-processing. */}
        <Shirt
          className="size-[46%]"
