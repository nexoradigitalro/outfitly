...
> **In progress:** Closet grid, category tabs, and ItemCard ship in the real Prism visual language (`apps/web/app/(app)/page.tsx`, `components/closet/`), wired to real `closet_items` queries/RLS — verified end-to-end (guest session → insert → persists across reload). Real capture also ships (`app/(app)/capture`, `hooks/use-add-closet-item.ts`): photo upload to `closet-original` (private bucket, signed URL for display via `next/image`), manual name/category/color entry, insert, redirect back to Closet — verified end-to-end with a real file upload. **Not yet built:** background removal, AI tagging/confidence, and the editable Item detail sheet (§12) — capture today is manual-only, that's the honest gap until an AI provider is chosen and its key configured. A dev-only "Seed demo items" action (`hooks/use-closet.ts`) still exists alongside real capture for fast demoing without a photo — delete it once it's no longer needed.
