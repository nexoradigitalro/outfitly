...
Consent step originally shipped gating **M0** itself. **Status update:** while only the team is testing (no real users), the `proxy.ts` gate in front of every Guest request is disabled and the simplified `ConsentForm` (one "Accept & Continue" + a separate optional marketing toggle, see ARCHITECTURE.md §10) now fires at **M-Auth** instead. This must flip back — gate reinstated before first write, not just before M-Auth — the moment anyone outside the team gets Guest access with real photos. `/consent` and `/legal/*` stay in the codebase either way. Basic info / style prefs / body photos / avatar generation are deferred to whichever milestone first consumes them (M1 Closet for basic info, later for avatar/try-on) rather than collected upfront with nothing yet using them.
...
Supabase project + core migrations (§8), RLS policies, **anonymous (Guest) auth only** — see note below, `packages/types` + `packages/api-client` skeleton, base app shell (bottom nav, route groups, dark theme tokens). GDPR consent (`ConsentForm`, `consent_events` logging — ARCHITECTURE.md §10) was built here but its enforcement gate is currently disabled for Guest (team-only testing) and re-fires at M-Auth — see the status note above and in §10.
...
> **In progress:** Closet grid, category tabs, ItemCard, real capture (photo upload -> `closet-original`, manual name/category/color, signed-URL display via `next/image`), and the Item Detail Sheet (edit name/category/color, favorite toggle, remove — soft delete via `archived_at`) are all real and wired to Supabase/RLS — full create/read/update/delete loop works end-to-end, verified against a **production build** on a real device, not just `localhost` (see `apps/web/CLAUDE.md` for why dev mode isn't trustworthy for real-device verification). **Not yet built:** background removal and AI tagging/confidence (§12) — capture and editing are manual-only until an AI provider is chosen and its key configured; that's the one honest gap left before M1 is functionally complete. A dev-only "Seed demo items" action (`hooks/use-closet.ts`) still exists alongside real capture for fast demoing without a photo — delete it once it's no longer needed.
...
Google OAuth + Magic Link screens, guest→account linking (`linkIdentity`, no data migration needed), onboarding gate enforcement, **and re-enabling the GDPR consent gate** (disabled for Guest since the "remove it for now" call during team-only testing — ARCHITECTURE.md §10) — real accounts must hit `ConsentForm` before creation, and Guest must be re-gated before this milestone is considered done if Guest access has gone out to anyone beyond the team by then. Ships once M1/M2 have proven the core loop is worth protecting behind a real identity. Apple Sign-In stays a reserved UI slot until native app work starts (App Store requires it there; not required for web).
...
|---|---|
| M0 | A guest session lands on an empty, correctly-themed app shell and can write closet data; RLS verified with a second anonymous session (no cross-account data leakage). *(Consent gate exists but is intentionally not enforced for Guest yet — team-only testing, see §10/§21.)* |
...
| M-Auth | A guest with an existing closet can sign in with Google/Magic Link and keep every item — same `profile_id`, zero data loss. If Guest access has reached anyone outside the team by this point, the consent gate is back on before this milestone counts as done. |
...
| Consent collected sloppily (single "agree to everything" box) is legally weak in the EU | Marketing stays a separate, optional, off-by-default toggle from the required Terms/Privacy acceptance; log every consent as an individually-timestamped, versioned `consent_events` row (ARCHITECTURE.md §8, §10) rather than a boolean |
| Guest write-access currently has no consent gate in front of it | Acceptable only while the team is the sole tester (explicit call, see §10/§21) — tracked, not forgotten: gate must be back on before any real user's Guest session can write real photos, at latest by M-Auth |
